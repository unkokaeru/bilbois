Conduct research -> report & presentation (sectioned per member)

### Members

| Role                     | Name                          |
| ------------------------ | ----------------------------- |
| Chair                    | Katie Arnold (**27620935**)   |
| Secretary                | William Fayers (**27378661**) |
| Report Coordinator       | Torin (**26424071**)          |
| Presentation Coordinator | Tom Ward (**26479576**)       |
| Research Coordinator     | Daniel Morris (**26700885**)  |
| IT Coordinator           | Alex Rushworth (**27554937**) |
| Designated HR            | Hayden Loyseau (**27635146**) |

### Report Requirements

- You should be able to teach the topic to another student in your year group **using the report as a textbook**.
- Include **worked examples** where possible.
- The report should form a **coherent whole**.
- Focus on the **mathematical/scientific** content of your project area.
- Short abstract, 2-3 pages of introduction, main body of text, a 1-3- page summary/conclusion, bibliography and appendices.
- Communal sections should be written as a group.
- Each chapter should show evidence of **research** and **critical thinking** and include **references (IEEE style)**.
- **Include meeting minutes as an appendix**.

### Presentation Requirements

- Typical individual contribution: **3 minutes**.
- An **introduction** and **conclusion** should also be included.
- Assume that the audience consists of the other students in your year group.
- A range of software is available: PowerPoint, Teams, Panopto etc.

Examples:
- [Group Presentation](https://uol.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=d59280d4-0e6b-4b4f-aaa9-afa40107afe1&start=1068.479007).
- [Individual Contribution](https://uol.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=cf8dea28-a916-4223-bf94-acda00dffa50&start=0).